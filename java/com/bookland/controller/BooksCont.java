...
public class BooksCont extends Main {
    ...
    public String search(Model model, @RequestParam(defaultValue = "") String search) {
        ...
        model.addAttribute("books", booksRepo.findAllByNameContaining(search));
        return "books";
    }
    ...
    @PostMapping("/add")
    public String BookAddNew(Model model, @RequestParam String name, @RequestParam String isbn, @RequestParam MultipartFile photo, @RequestParam MultipartFile[] photos, @RequestParam String date, @RequestParam int quantity, @RequestParam String description, @RequestParam Category category, @RequestParam Genre genre, @RequestParam String author) {
        Books book;
        if (photos != null && !Objects.requireNonNull(photos[0].getOriginalFilename()).isEmpty()) {
            try {
                ...
                if (photo != null && !Objects.requireNonNull(photo.getOriginalFilename()).isEmpty()) {
                    ...
                    result = "books/" + uuidFile + "_" + photo.getOriginalFilename();
                    photo.transferTo(new File(uploadImg + "/" + result));
                }

                book = new Books(name, isbn, date, result, result_photos);
    ...
    public String BookEditOld(Model model, @RequestParam String name, @RequestParam String isbn, @RequestParam MultipartFile photo, @RequestParam MultipartFile[] photos, @RequestParam String date, @RequestParam int quantity, @RequestParam String description, @RequestParam Category category, @RequestParam Genre genre, @RequestParam String author, @PathVariable Long id) {
        ...
        try {
            if (photo != null && !Objects.requireNonNull(photo.getOriginalFilename()).isEmpty()) {
                ...
                File uploadDir = new File(uploadImg);
                if (!uploadDir.exists()) uploadDir.mkdir();
                String result = "books/" + uuidFile + "_" + photo.getOriginalFilename();
                photo.transferTo(new File(uploadImg + "/" + result));
                book.setPhoto(result);
            }
        } catch (IOException e) {
            AddAttributes(model);
            model.addAttribute("message", "Ошибка, некорректный данные!");
